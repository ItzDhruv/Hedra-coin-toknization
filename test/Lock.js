...
describe("Token-Launchpad Contract", function () {
	...
	it("should allow user to sell tokens back to token owner", async function () {
		// Launch token
		await token.connect(owner).launchToken("TestToken", "TTK", 1000000, ethers.parseEther("0.01"));
		...
		// Owner approves TokenLaunchpad to transfer tokens on their behalf
		...
		// Owner needs to have ETH to send when someone sells back tokens
		...
		// Check user token balance after sell
		const userBalance = await erc20.balanceOf(user1.address);
		expect(userBalance).to.equal(buyAmount - sellAmount);
	
		// Check ETH after sell (accounting for gas)
